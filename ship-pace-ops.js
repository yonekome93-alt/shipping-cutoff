...
(function(){
  ...
  async function dismissReachedWave(){const c=opsCompletionCandidate;if(c){const item=state.waves.find(x=>x.id===c.waveId);if(item)item.completedAt=null;if(!state.completionDismissals.some(x=>x.waveId===c.waveId&&x.targetMinute===c.targetMinute))state.completionDismissals.push({...c,at:new Date().toISOString()})}$('completionModal').classList.remove('open');opsCompletionCandidate=null;await saveState('shipping_completion_deferred');$('quickResult').insertAdjacentHTML('beforeend','<p>進捗は保存し、便は未完了のままです。</p>')}
