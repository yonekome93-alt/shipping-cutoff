...
(function(){
  ...
  function renderDashboard(){
    ...
    const nextTarget=opsNextTarget(),cp=opsCheckpoint(nextTarget),progressTargets=opsTargetMinutes(),allProgressEntered=progressTargets.length>0&&progressTargets.every(t=>Boolean(opsCheckpoint(t))),progressInputStatus=allProgressEntered?'本日の進捗入力は完了':`${clock(nextTarget)}　${cp?'✓ 入力済み':nextTarget<=nowMinutes()?'⚠ 未入力':'― これから'}`,latestProgress=latest?`${clock(latest.targetMinute)}時点・累計${Number(latest.totalCompleted).toLocaleString()}店舗`:'まだありません',tone=diff>0?'ahead':diff<0?'behind':'on-plan',requirementText=requirement.status==='overdue'?'算出対象外':Number.isFinite(required)?`${required}名`:'算出不可',requirementDetail=requirement.status==='overdue'?`${requirement.detail}。残作業を確認してください。`:action;$('currentProgressDashboard').innerHTML=`<div class="ops-dashboard-head"><div><div class="eyebrow">CURRENT PROGRESS</div><h2>現在の進捗</h2></div><span class="ops-asof">${latest?clock(target)+'時点':'実績待ち'}</span></div><div class="ops-variance ${tone}">${opsDiffLabel(diff,'計画')}</div><div class="ops-dashboard-grid"><div class="ops-kpi ops-kpi-plan"><small>計画｜実績（店舗）</small><strong>${plan.toLocaleString()}｜${actual.toLocaleString()}</strong></div><div class="ops-kpi"><small>累計 全体生産性</small><strong>${productivity===null?'実績待ち':productivity.toFixed(1)+' 店舗/人時'}</strong></div><div class="ops-kpi"><small>完了見込み</small><strong>${finish}</strong></div><div class="ops-kpi"><small>現在人数｜必要人数</small><strong>${current}名｜${requirementText}</strong></div><div class="ops-kpi"><small>次に入力が必要な進捗</small><strong>${progressInputStatus}</strong><small class="ops-kpi-note">最新の有効な進捗：${latestProgress}</small></div></div><div class="ops-action ${requirement.status==='overdue'||requirement.gap>0?'warn':''}">${requirementDetail}</div>`;
  }
  ...
  function opsShowBuild(){let badge=$('shipPaceBuild');if(!badge){badge=document.createElement('span');badge.id='shipPaceBuild';badge.style.cssText='font-size:10px;font-weight:900;color:#667085;white-space:nowrap';document.querySelector('header .ops-status')?.appendChild(badge)}if(badge)badge.textContent='v47'}
